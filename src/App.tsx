    ...
    <div className="container mx-auto space-y-8 py-8">
      ...
      <form className="space-y-8 divide-y divide-gray-200">
        <div className="space-y-8 divide-y divide-gray-200 sm:space-y-5">
          <div className="space-y-6 sm:space-y-5">
            <div>
              <h3 className="text-lg font-medium leading-6 text-gray-900">Migration</h3>
              <p className="mt-1 max-w-2xl text-sm text-gray-500">
                This wizard will guide you through the migration of your "Liquid Investigations: Elbrus" instance.
              </p>
            </div>
        ...
        <div className="pt-5">
          <div className="flex justify-end">
            ...
            <Button onClick={handleRun} label="Go" disabled={password === ""} className="ml-3 inline-flex justify-center" />
          </div>
        </div>
      </form>
      ...
      {errors.length > 0 && <ul>
        {errors.map(({id, handler, message}) =>
          <li key={id} className="text-red-400"><b>{handler}</b>: {message}</li>)}
      </ul>}

      {uname && uname.type === "success" && <p>Host Uname: {uname.uname}</p>}

      {wgConfig && wgConfig.type === "success" && <pre>{wgConfig.config_file}</pre>}
    </div>
  );
}
